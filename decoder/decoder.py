...
class Decoder:
    ...
    def load(self, file_name):
        ...
        distinct_data_items_cat48 = set()
        distinct_data_items_cat21 = set()
        for element in tqdm(self.splited_data[:100]):
            cat, length, data = element
            if cat == 48:
                data_items_to_decode = decode_cat48(cat, length, data.bin)
                distinct_data_items_cat48.update(data_items_to_decode)
            elif cat == 21:
                data_items_to_decode, decoded_data, end_pos = decode_cat21(cat, length, data)
                distinct_data_items_cat21.update(data_items_to_decode)


        print(f"Distinct CAT48 data items: {sorted(distinct_data_items_cat48)}")
        print(f"Distinct CAT21 data items (FRNs): {sorted(distinct_data_items_cat21)}")