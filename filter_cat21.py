...
def filter_csv():
    ...
    try:
        ...
        print(
            "Asegúrate de ejecutar primero el script 'decoder' para generar este archivo."
        )
        return
    ...
    if "GBS" not in df.columns:
        ...
        print(
            "Asegúrate de haber modificado 'cat21.py' y 'decoder.py' y regenerado el CSV."
        )
    ...
    df_filtered["Latitude (deg)"] = pd.to_numeric(
        df_filtered["Latitude (deg)"], errors="coerce"
    )
    df_filtered["Longitude (deg)"] = pd.to_numeric(
        df_filtered["Longitude (deg)"], errors="coerce"
    )
