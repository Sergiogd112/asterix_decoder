...
def extract_actual_mode3a_data():
    """Extract actual Mode-3/A bytes from ASTERIX data to see what's being processed."""
    ...
    while pos < len(data) and count < 3:
        ...
        if cat == 48 and length > 0 and pos + 3 + length <= len(data):
            ...
            # Check if Mode 3/A (FRN 5) is present
            if len(fspec_data) >= 5 and fspec_data[4]:  # FRN 5 is index 4
                ...
                # Skip FRN 1: Data Source ID (2 bytes)
                if fspec_data[0]:
                    mode3a_start += 16
                    print(f"Skipping DSI: +16 bits")

                # Skip FRN 2: Target Report Descriptor (variable)
                if fspec_data[1]:
                    trd_bits = bits[mode3a_start : mode3a_start + 8]
                    mode3a_start += 8
                    fx = trd_bits[7]
                    print(f"Skipping TRD: +8 bits, FX={fx}")
                    if fx:
                        # Skip extension
                        ext_bits = bits[mode3a_start : mode3a_start + 8]
                        mode3a_start += 8
                        print(f"Skipping TRD extension: +8 bits")

                # Skip FRN 3: Track Number (2 bytes)
                if fspec_data[2]:
                    mode3a_start += 16
                    print(f"Skipping Track Number: +16 bits")

                # Skip FRN 4: Service Identification (1 byte)
                if fspec_data[3]:
                    mode3a_start += 8
                    print(f"Skipping Service ID: +8 bits")

                # Now extract Mode 3/A (2 bytes)
                if mode3a_start + 16 <= len(bits):
                    mode3a_bits = bits[mode3a_start : mode3a_start + 16]
                    mode3a_val = mode3a_bits.uint
                    print(f"Mode 3/A raw bytes: {mode3a_bits.hex}")
                    print(f"Mode 3/A raw value: {mode3a_val:04X} ({mode3a_val})")
                    print(f"Mode 3/A raw bits: {mode3a_val:016b}")

                    # Test both implementations
                    # Old implementation (bits 0-11)
                    old_code = mode3a_val & 0x0FFF
                    ...
                    for i in range(0, 12, 3):
                        bit0 = (mode3a_val >> (15 - (i + 4))) & 1
                        bit1 = (mode3a_val >> (15 - (i + 5))) & 1
                        bit2 = (mode3a_val >> (15 - (i + 6))) & 1
